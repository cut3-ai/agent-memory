...
const cliPath = fileURLToPath(new URL('../legacy/cli.js', import.meta.url));
...
test('CLI keeps raw observations outside the public run tree', async () => {
  ...
  try {
    ...
    const publicRun = path.join(fixtureRoot, 'lab', 'runs', 'legacy', runId);
    const privateRun = path.join(fixtureRoot, 'lab', 'runs', '.private-legacy', runId);
