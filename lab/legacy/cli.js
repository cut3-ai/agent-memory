...
export async function main(argv = process.argv.slice(2)) {
  const { values, positionals } = parseArgs({
    ...
    options: {
      ...
      out: { type: 'string', short: 'o', default: 'lab/runs/legacy' },
      'private-out': { type: 'string', default: 'lab/runs/.private-legacy' },
...
function helpText() {
  return [
    'Usage: node lab/legacy/cli.js --input <workspaces.jsonl> [options]',
    '',
    'Options:',
    '  -i, --input       Source JSONL dataset (required)',
    '  -o, --out         Public sanitized artifact root (default: lab/runs/legacy)',
    '      --private-out Private raw artifact root (default: lab/runs/.private-legacy)',
