...
import { writeNavigationIndex } from '../../src/library/index.js';
...
import { assertPublicArtifact, inspectPublicArtifact } from '../../src/memory/privacy.js';
...
export async function runMemoryPipeline(inputText, options = {}) {
  ...
  const outputRoot = path.resolve(
    options.outputRoot ?? path.join(repositoryRoot, 'lab', 'runs', 'census'),
  );
  ...
  // Census is an offline observer. Production index mutation must be an
  // explicit operator action, never a side effect of running a dataset lab.
